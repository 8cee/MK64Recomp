...
// Accent do projeto (verde-banana MK64) para controles neutros pressionados.
private const val ACCENT = 0xFFF2C94C.toInt()
...
/**
 * Gamepad virtual do port Android do MK64Recomp Android — v2 "dark glass".
 *
 ...
 * verde MK64). O HUD entra/sai com fade+deslize e aparece sozinho quando o
...
class TouchControlsView @JvmOverloads constructor(
    ...
    companion object {
        ...
        // usado no MK64; os ids são mantidos para casar com o enum nativo.
        ...
        @Volatile
        private var instance: TouchControlsView? = null
    ...
    private fun nativeInit() {
        Diagnostics.info("MK64-style touch controls initialized")
    }

    private fun nativeButton(id: Int, pressed: Boolean) {
        if (id == BTN_MENU) {
            if (pressed) Diagnostics.info("Touch MENU pressed")
            return
        }
        NativeBridge.setButton(id, pressed)
    }

    private fun nativeAxis(x: Float, y: Float) {
        NativeBridge.setStick(x, y)
    }

    private fun nativeIsGameStarted(): Boolean = true
